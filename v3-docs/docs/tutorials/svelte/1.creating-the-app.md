...
By default, when using Svelte with previous Meteor versions, reactivity is handled by packages like `zodern:melte` (which integrates Svelte with Meteor). This allows real-time updates of the users screen as data changes in the database without them having to manually refresh. You can read more about it [here](https://atmospherejs.com/zodern/melte).

> You can read more about packages [here](https://docs.meteor.com/packages/).

::: warning
`zodern:melte` package is not compatible with Meteor Rspack apps (Meteor 3.4+ using the rspack package). While this means losing some of the sugar helpers provided by the library, it also brings advantages, such as support for the latest versions of Svelte 5 and newer ecosystem implementations. The community may help implement these helpers decoupled from Meteor's build system for later adoption with Meteor-Rspack.
:::
