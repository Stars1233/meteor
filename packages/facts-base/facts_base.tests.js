...
Tinytest.add(
  "facts-base - resetServerFacts on already empty state is a no-op",
  (test) => {
    Facts.resetServerFacts();
    Facts.resetServerFacts();
...
Tinytest.add(
  "facts-base - incrementServerFact creates entry for new package",
  (test) => {
    Facts.resetServerFacts();
...
Tinytest.add(
  "facts-base - incrementServerFact adds new fact to existing package",
  (test) => {
    Facts.resetServerFacts();
...
Tinytest.add(
  "facts-base - incrementServerFact accumulates on existing fact",
  (test) => {
    Facts.resetServerFacts();
...
Tinytest.add(
  "facts-base - incrementServerFact handles negative increment",
  (test) => {
    Facts.resetServerFacts();
    Facts.incrementServerFact("pkg", "counter", 10);
...
Tinytest.add(
  "facts-base - incrementServerFact keeps packages independent",
  (test) => {
    Facts.resetServerFacts();
...
Tinytest.add(
  "facts-base - notifies subscriptions with added when package is new",
  (test) => {
    Facts.resetServerFacts();
    const sub = mockSub();
...
Tinytest.add(
  "facts-base - notifies subscriptions with changed when fact is updated",
  (test) => {
    Facts.resetServerFacts();
    const sub = mockSub();
...
Tinytest.add(
  "facts-base - notifies all active subscriptions on increment",
  (test) => {
    Facts.resetServerFacts();
    const sub1 = mockSub();
    const sub2 = mockSub();
...
Tinytest.add(
  "facts-base - incrementServerFact works with no active subscriptions",
  (test) => {
    Facts.resetServerFacts();
    Facts._setActiveSubscriptions([]);
